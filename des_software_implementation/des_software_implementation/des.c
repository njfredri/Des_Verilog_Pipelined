...
int PBox(int pos, int text) //another permutation
{
    int i;
    for (i = 0; i < 32; i++)
    {
...
{
    out = fopen("cipher.txt", "ab+"); //initial permutation
    for (int i = 0; i < 64; i++) {
        initialPermutation(i, plain[i]);
    }

    for (int i = 0; i < 32; i++) { //left0 = half from 0 to 31
        LEFT[0][i] = IPtext[i];
    }
    ...
    for (int k = 1; k < 17; k++)
    {
        cipher(k, 0); //call cipher function for index k
